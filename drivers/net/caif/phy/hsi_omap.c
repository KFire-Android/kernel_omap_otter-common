...
static inline void cfhsi_omap_write_cb(struct hsi_device *dev,
					unsigned int size)
{
	struct cfhsi_omap *cfhsi = NULL;
	struct list_head *list_node;
...
static void hsi_proto_release(struct device *dev)
{
	...
	list_for_each_safe(list_node, n, &cfhsi_dev_list) {
		...
		if (&cfhsi->pdev.dev == dev) {
			...
			if (res) {
				printk(KERN_WARNING \
				"hsi_proto_release: failed to set WAKE: %d\n",
				res);
			}
			hsi_close(cfhsi->hsi_dev);
			/* Free memory. */
...
static int hsi_proto_probe(struct hsi_device *dev)
{
	...
	if (res) {
		printk(KERN_WARNING \
		"hsi_proto_probe: failed to open HSI device: %d.\n", res);
		goto err_hsi_open;
	}
	...
	if (res) {
		printk(KERN_WARNING \
		"hsi_proto_probe: failed to configure TX: %d.\n", res);
		goto err_hsi_ioctl;
	}
	...
	if (res) {
		printk(KERN_WARNING \
		"hsi_proto_probe: failed to configure RX: %d.\n", res);
		goto err_hsi_ioctl;
	}
