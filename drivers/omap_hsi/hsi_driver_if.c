...
static int hsi_set_rx(struct hsi_port *sport, struct hsr_ctx *cfg)
{
	struct hsi_dev *hsi_ctrl = sport->hsi_controller;
	void __iomem *base = hsi_ctrl->base;
	int port = sport->port_number;
...
static void hsi_get_rx(struct hsi_port *sport, struct hsr_ctx *cfg)
{
	struct hsi_dev *hsi_ctrl = sport->hsi_controller;
	void __iomem *base = hsi_ctrl->base;
	int port = sport->port_number;
...
static void hsi_get_tx(struct hsi_port *sport, struct hst_ctx *cfg)
{
	struct hsi_dev *hsi_ctrl = sport->hsi_controller;
	void __iomem *base = hsi_ctrl->base;
	int port = sport->port_number;
...
int hsi_open(struct hsi_device *dev)
{
	...
	if (!ch->read_done || !ch->write_done) {
		dev_err(dev->device.parent,
			"Trying to open with no (read/write) callbacks registered\n");
		return -EINVAL;
	}
...
int hsi_write(struct hsi_device *dev, u32 *addr, unsigned int size)
{
	...
	if (ch->write_data.addr != NULL) {
		dev_err(hsi_ctrl->dev, "# Invalid request - Write operation pending port %d channel %d\n",
...
int hsi_read(struct hsi_device *dev, u32 *addr, unsigned int size)
{
	...
	if (unlikely(!dev->ch || !addr || (size <= 0))) {
		dev_err(dev->device.parent, "Wrong parameters hsi_device %p data %p count %d",
			dev, addr, size);
		return -EINVAL;
	}
	...
	if (ch->read_data.addr != NULL) {
		dev_err(hsi_ctrl->dev, "# Invalid request - Read operation pending port %d channel %d\n",
					ch->hsi_port->port_number,
					ch->channel_number);
		err = -EINVAL;
		goto done;
...
static int __hsi_write_cancel(struct hsi_channel *ch)
{
	int err = -ENODATA;
...
static int __hsi_read_cancel(struct hsi_channel *ch)
{
	int err = -ENODATA;
	struct hsi_dev *hsi_ctrl = ch->hsi_port->hsi_controller;
	...
	case HSI_IOCTL_ACWAKE_DOWN:
		...
		if (unlikely(pport->acwake_status !=
			...
			dev_warn(hsi_ctrl->dev,
				"ACWAKE shadow register mismatch acwake_status: 0x%x, HSI_SYS_WAKE_REG: 0x%x",
