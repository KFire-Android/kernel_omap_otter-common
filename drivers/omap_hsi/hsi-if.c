...
void if_hsi_set_clk_forced_on(int ch)
{
	struct if_hsi_channel *channel;
	channel = &hsi_iface.channels[ch];

	spin_lock_bh(&hsi_iface.lock);
	hsi_ioctl(channel->dev, HSI_IOCTL_SET_CLK_FORCE_ON, NULL);
	spin_unlock_bh(&hsi_iface.lock);
}

void if_hsi_set_clk_dynamic(int ch)
{
	struct if_hsi_channel *channel;
	channel = &hsi_iface.channels[ch];

	spin_lock_bh(&hsi_iface.lock);
	hsi_ioctl(channel->dev, HSI_IOCTL_SET_CLK_DYNAMIC, NULL);
	spin_unlock_bh(&hsi_iface.lock);
}

void if_hsi_set_hsi_latency(int ch, int lat)
{
	struct if_hsi_channel *channel;
	channel = &hsi_iface.channels[ch];

	spin_lock_bh(&hsi_iface.lock);
	hsi_ioctl(channel->dev, HSI_IOCTL_SET_HSI_LATENCY, &lat);
	spin_unlock_bh(&hsi_iface.lock);
}
...
void if_hsi_set_mpu_opp(int ch, unsigned long mpu_fclk)
{
	...
	spin_lock_bh(&hsi_iface.lock);
	hsi_ioctl(channel->dev, HSI_IOCTL_SET_MPU_OPP, &mpu_fclk);
	spin_unlock_bh(&hsi_iface.lock);
}
...
void if_hsi_get_mpu_opp(int ch, unsigned long *mpu_fclk)
{
	struct if_hsi_channel *channel;
	channel = &hsi_iface.channels[ch];
	hsi_ioctl(channel->dev, HSI_IOCTL_GET_MPU_OPP, mpu_fclk);
}
		      ...
		      unsigned int num_channels)
{
	...
	for (i = 0; (i < num_channels) && channels_map[i]; i++) {
		...
		if ((channels_map[i] - 1) < HSI_MAX_CHAR_DEV_ID)
			...
			pr_err("Channel %d cannot be handled by the HSI driver.\n",
			       channels_map[i]);
			return -EINVAL;
		}
	...
	if (hsi_iface.init_chan_map) {
		ret = -ENXIO;
		pr_err("HSI: Some channels could not be registered (out of range or already registered?)\n");
	}
	return ret;
}
