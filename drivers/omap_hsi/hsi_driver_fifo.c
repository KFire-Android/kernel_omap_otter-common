...
fifo_id_bk:
	if (unlikely(err < 0)) {
		fifo_index = err;
		dev_err(hsi_ctrl->dev, "Cannot map a FIFO to the requested params: channel:%d, port:%d; ERR=%d\n",
			channel, port, err);
	}
...
fifo_id_bk:
	if (unlikely(err < 0))
		dev_err(hsi_ctrl->dev, "Cannot map a channel / port to the requested params: fifo:%d; ERR=%d\n",
			fifo, err);
...
int hsi_fifo_mapping(struct hsi_dev *hsi_ctrl, unsigned int mtype)
{
	...
	if ((mtype == HSI_FIFO_MAPPING_ALL_PORT1) ||
		...
		if (hsi_ctrl->fifo_mapping_strategy == HSI_FIFO_MAPPING_UNDEF)
			dev_dbg(hsi_ctrl->dev, "Fifo mapping : All FIFOs for Port %d\n",
				port + 1);
		...
		if (hsi_ctrl->fifo_mapping_strategy == HSI_FIFO_MAPPING_UNDEF)
			dev_dbg(hsi_ctrl->dev, "Fifo mapping : 8 FIFOs per Port (SSI compatible mode)\n");
	} else {
