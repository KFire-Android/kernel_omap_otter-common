			 ...
			 unsigned int size)
{
	...
	if (lch < 0) {
		...
		dev_dbg(hsi_ctrl->dev, "Allocated DMA channel %d for write on HSI channel %d.\n",
			lch, hsi_channel->channel_number);
	}
	...
	if (hsi_driver_device_is_hsi(to_platform_device(hsi_ctrl->dev))) {
		...
		if (unlikely(fifo < 0)) {
			dev_err(hsi_ctrl->dev, "No valid FIFO id for DMA transfer to FIFO.\n");
			return -EFAULT;
		}
			...
			unsigned int count)
{
	...
	if (lch < 0) {
		...
		dev_dbg(hsi_ctrl->dev, "Allocated DMA channel %d for read on HSI channel %d.\n",
			lch, hsi_channel->channel_number);
	}
	...
	if (hsi_driver_device_is_hsi(to_platform_device(hsi_ctrl->dev))) {
		fifo = hsi_fifo_get_id(hsi_ctrl, channel, port);
		if (unlikely(fifo < 0)) {
			dev_err(hsi_ctrl->dev, "No valid FIFO id for DMA transfer from FIFO.\n");
			return -EFAULT;
		}
...
int hsi_driver_cancel_write_dma(struct hsi_channel *hsi_ch)
{
	...
	if (lch < 0) {
		dev_err(&hsi_ch->dev->device, "No DMA channel found for HSI channel %d\n",
			 hsi_ch->channel_number);
		return -ENXIO;
	}
	ccr = hsi_inw(hsi_ctrl->base, HSI_GDD_CCR_REG(lch));
	if (!(ccr & HSI_CCR_ENABLE)) {
		dev_dbg(&hsi_ch->dev->device, "Write cancel on not enabled logical channel %d CCR REG 0x%04X\n",
			lch, ccr);
	}
...
int hsi_driver_cancel_read_dma(struct hsi_channel *hsi_ch)
{
	...
	ccr = hsi_inw(hsi_ctrl->base, HSI_GDD_CCR_REG(lch));
	if (!(ccr & HSI_CCR_ENABLE)) {
		dev_dbg(&hsi_ch->dev->device, "Read cancel on not enabled logical channel %d CCR REG 0x%04X\n",
			lch, ccr);
	}
 ...
 * Return 0 on success and < 0 on error.
 */
static int hsi_get_info_from_gdd_lch(struct hsi_dev *hsi_ctrl, unsigned int lch,
...
static void do_hsi_gdd_lch(struct hsi_dev *hsi_ctrl, unsigned int gdd_lch)
{
	...
	if (hsi_get_info_from_gdd_lch(hsi_ctrl, gdd_lch, &port_i, &channel,
				      &is_read_path) < 0) {
		dev_err(hsi_ctrl->dev, "Unable to match the DMA channel %d with an HSI channel\n",
			gdd_lch);
		return;
	} else {
		port  = hsi_ctrl->hsi_port[port_i].port_number;
		dev_dbg(hsi_ctrl->dev, "DMA event on gdd_lch=%d => port=%d, channel=%d, read=%d\n",
			gdd_lch, port, channel, is_read_path);
	}
	...
	if (!(gdd_csr & HSI_CSR_TOUT)) {
		if (is_read_path) {	/* Read path */
			...
			dev_dbg(hsi_ctrl->dev, "Calling ch %d read callback (size %d).\n",
				channel,  size/4);
			...
			if (hsi_driver_device_is_hsi(pdev)) {
				fifo = hsi_fifo_get_id(hsi_ctrl, channel, port);
				if (unlikely(fifo < 0)) {
					dev_err(hsi_ctrl->dev, "No valid FIFO id found for channel %d.\n",
						channel);
					return;
				}
				fifo_words_avail =
				...
				if (fifo_words_avail)
					dev_dbg(hsi_ctrl->dev,
						"FIFO %d not empty after DMA copy, remaining %d/%d frames\n",
			...
			dev_dbg(hsi_ctrl->dev, "Calling ch %d write callback (size %d).\n",
				channel, size/4);
			spin_unlock(&hsi_ctrl->lock);
		...
		dev_err(hsi_ctrl->dev, "Time-out overflow Error on GDD transfer on gdd channel %d\n",
			gdd_lch);
		spin_unlock(&hsi_ctrl->lock);
