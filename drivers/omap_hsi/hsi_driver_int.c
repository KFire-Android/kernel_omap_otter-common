...
bool hsi_is_port_transfer_ongoing(struct hsi_port *pport)
{
	...
	for (ch = 0; ch < pport->max_ch; ch++)
		if (hsi_is_channel_transfer_ongoing(&pport->hsi_channel[ch])) {
			dev_dbg(hsi_ctrl->dev, "Port %d; channel %d transfer ongoing\n",
...
bool hsi_is_hsi_port_busy(struct hsi_port *pport)
{
	...
	if (cur_cawake || pport->acwake_status) {
		dev_dbg(hsi_ctrl->dev, "Port %d: WAKE status: acwake_status %d, cur_cawake %d",
			pport->port_number, pport->acwake_status, cur_cawake);
		return true;
	}

	for (ch = 0; ch < pport->max_ch; ch++)
		if (hsi_is_channel_busy(&pport->hsi_channel[ch])) {
			dev_dbg(hsi_ctrl->dev, "Port %d; channel %d busy\n",
				pport->port_number, ch);
			return true;
		}
...
bool hsi_is_hst_port_busy(struct hsi_port *pport)
{
	...
	if (txstateval != HSI_HST_TXSTATE_IDLE) {
		dev_dbg(pport->hsi_controller->dev, "HST port %d busy, TXSTATE=%d\n",
			port, txstateval);
		return true;
	}
...
int hsi_driver_cancel_write_interrupt(struct hsi_channel *ch)
{
	...
	if (!(status_reg & HSI_HST_DATAACCEPT(channel))) {
		dev_dbg(&ch->dev->device, "Write cancel on not enabled channel %d ENABLE REG 0x%08X",
			channel, status_reg);
	}
	status_reg &= hsi_inl(base,
...
int hsi_driver_cancel_read_interrupt(struct hsi_channel *ch)
{
	...
	if (!(status_reg & HSI_HSR_DATAAVAILABLE(channel))) {
		dev_dbg(&ch->dev->device, "Read cancel on not enabled channel %d ENABLE REG 0x%08X",
			channel, status_reg);
	}
	status_reg &= hsi_inl(base,
			HSI_SYS_MPU_STATUS_CH_REG(port, p->n_irq, channel));
	...
	if (cawake_status) {
		...
		*/
		...
		if (hsi_is_dma_read_int_pending(hsi_ctrl)) {
			dev_dbg(hsi_ctrl->dev, "Pending DMA Read interrupt before CAWAKE->L, exiting Interrupt tasklet.\n");
			return -EAGAIN;
		}
		if (unlikely(!pport->cawake_status)) {
			dev_warn(hsi_ctrl->dev,
...
static u32 hsi_process_int_event(struct hsi_port *pport)
{
	...
	if (hsi_driver_is_interrupt_pending(pport, HSI_CAWAKEDETECTED, true) &&
	    (status_reg & HSI_CAWAKEDETECTED)) {
		dev_warn(pport->hsi_controller->dev, "New CAWAKE interrupt detected during interrupt processing\n");
...
int __devinit hsi_mpu_init(struct hsi_port *hsi_p, const char *irq_name)
{
	...
	err = request_irq(hsi_p->irq, hsi_mpu_handler,
			...
			irq_name, hsi_p);
	if (err < 0) {
		dev_err(hsi_p->hsi_controller->dev, "FAILED to MPU request IRQ (%d) on port %d",
			hsi_p->irq, hsi_p->port_number);
		return -EBUSY;
	}
