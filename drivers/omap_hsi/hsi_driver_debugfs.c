...
#define MAXWORDS 4
	...
	if (hsi_driver_device_is_hsi(pdev)) {
		if (nwords != 3) {
			dev_warn(hsi_ctrl->dev, "HSI counters write usage: echo \"FT TB FB\" > counters\n");
			ret = -EINVAL;
			goto hsi_cnt_w_bk1;
		}

		if (kstrtoul(words[0], 0, &ft) ||
		    kstrtoul(words[1], 0, &tb) ||
		    kstrtoul(words[2], 0, &fb)) {
		...
		if (nwords != 1) {
			dev_warn(hsi_ctrl->dev, "HSI counters write usage: echo \"timeout\" > counters\n");
			ret = -EINVAL;
			goto hsi_cnt_w_bk1;
		}
		if (kstrtoul(words[0], 0, &reg)) {
			dev_warn(hsi_ctrl->dev, "Not an unsigned long\n");
			ret = -EINVAL;
			goto hsi_cnt_w_bk1;
		}
