...
/* Define the platform data here for now. Later move it to dts files */
...
static int qt602240_probe(struct i2c_client *client,
		const struct i2c_device_id *id)
{
	...
	/* Use driver internal platform data definition until support for passing
	 * via DT is added later on
	 */
	client->dev.platform_data = &ts_platform_data;

	/* hardcode IRQ for DRA7-evm, for now */
	client->irq = 151;
