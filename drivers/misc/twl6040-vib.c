...
struct vib_data {
	...
	struct twl6040_vibra_data *pdata;
...
static void twl6040_vib_power_work(struct work_struct *work)
{
	...
	if (misc_data->powered) {
		twl6040_power(misc_data->twl6040, 0);
...
static int twl6040_vib_power(bool on)
{
	...
	if (on) {
		...
		ret = twl6040_power(misc_data->twl6040, 1);
		if (ret)
			goto out;
...
static int vib_probe(struct platform_device *pdev)
{
	...
	misc_data->vib_irq = platform_get_irq(pdev, 0);
	if (misc_data->vib_irq < 0) {
		pr_err("%s: invalid vibrator overcurrent irq\n", __func__);
		ret = -EINVAL;
		goto err2;
	}

	ret = request_threaded_irq(misc_data->vib_irq, NULL,
...
static int vib_remove(struct platform_device *pdev)
{
	...
	free_irq(misc_data->vib_irq, data);
	timed_output_dev_unregister(&data->dev);
	kfree(data);
