...
static void hdcp_wq_step2_authentication(void)
{
	...
	if (status == -HDCP_CANCELLED_AUTH) {
		...
		HDCP_INFO("(Repeater) authentication step 2 "
				 "successful\n");
...
static void hdcp_wq_authentication_failure(void)
{
	...
	if (hdcp.retry_cnt && (hdcp.hdmi_state != HDMI_STOPPED)) {
		if (hdcp.retry_cnt < HDCP_INFINITE_REAUTH) {
			hdcp.retry_cnt--;
			HDCP_INFO("authentication failed - "
					 "retrying, attempts=%d\n",
			       hdcp.retry_cnt);
		} else
			HDCP_INFO("authentication failed - "
...
static void omap4_hdcp_work_queue(struct work_struct *work)
{
	...
	case HDCP_WAIT_KSV_LIST:
		...
		/* Timeout */
			...
			HDCP_INFO("BCAPS polling timeout\n");
			hdcp_wq_authentication_failure();
		}
		break;
	...
	default:
		HDCP_ERR("error - unknow HDCP state\n");
		break;
	}
	...
	HDCP_DBG("hdcp_work_queue() - END - %u hdmi=%d hdcp=%d auth=%d "
		 "evt=%x %d ",
		 jiffies_to_msecs(jiffies),
		 hdcp.hdmi_state,
		 hdcp.hdcp_state,
...
static struct delayed_work *hdcp_submit_work(int event, int delay)
{
	...
	if (work) {
		...
		HDCP_WARN("Cannot allocate memory to "
				    "create work\n");
		return 0;
	}
...
static void hdcp_cancel_work(struct delayed_work **work)
{
	...
	if (*work) {
		...
		if (ret != 1) {
			...
			HDCP_INFO("Canceling work failed - "
...
static long omap4_hdcp_enable_ctl(void __user *argp)
{
	...
	if (hdcp.en_ctrl == 0) {
		...
		if (hdcp.en_ctrl == 0) {
			HDCP_ERR("Cannot allocate memory for HDCP"
	...
	if (copy_from_user(hdcp.en_ctrl, argp,
		...
		HDCP_WARN("Error copying from user space "
				    "- enable ioctl\n");
		return -EFAULT;
	}
...
static long omap4_hdcp_wait_event_ctl(void __user *argp)
{
	...
	if (copy_from_user(&ctrl, argp,
		...
		HDCP_ERR("Error copying from user space"
				    " - wait ioctl");
		return -EFAULT;
	}
	...
	if (copy_to_user(argp, &ctrl,
			 sizeof(struct hdcp_wait_control))) {
		HDCP_ERR("Error copying to user space -"
				    " wait ioctl");
		return -EFAULT;
	}
...
static long hdcp_ioctl(struct file *fd, unsigned int cmd, unsigned long arg)
{
	...
	case HDCP_QUERY_STATUS:
		...
		HDCP_INFO("%s ==> HDCP_QUERY_STATUS: %d \n", __func__, status);
		/* Store output data to output pointer */
	...
	default:
		HDCP_ERR("HDCP: invalid ioctl cmd received: %x\n", cmd);
		return -ENOTTY;
	} /* End switch */
...
static void hdcp_irq_cb(int status)
{
	...
	if (intr == KSVACCESSINT) {
		cancel_delayed_work(&hdcp.hdcp_work->dwork);
			   ...
			   uint32_t out_key[DESHDCP_KEY_SIZE])
{
	...
	HDCP_INFO("Encrypting HDCP keys...");
...
static long hdcp_encrypt_key_ctl(void __user *argp)
{
	...
	HDCP_INFO("hdcp_ioctl() - ENCRYPT KEY %u", jiffies_to_msecs(jiffies));

	mutex_lock(&hdcp.lock);

	if (hdcp.hdcp_state != HDCP_DISABLED) {
		HDCP_ERR("Cannot encrypt keys while HDCP "
				 "is enabled\n");
		mutex_unlock(&hdcp.lock);
		return -EFAULT;
	}
	...
	if (ctrl == 0) {
		HDCP_WARN("Cannot allocate memory for HDCP"
...
static int hdcp_wq_start_authentication(void)
{
	...
	if (ip_data->ops->hdcp_enable) {
		status = ip_data->ops->hdcp_enable(ip_data);
	} else
		status = -EINVAL;
...
static void hdcp_work_queue(struct work_struct *work)
{
	struct hdcp_worker_data *d = container_of(work, typeof(*d),
						  dwork.work);
	...
	HDCP_INFO("%s authentication, %s: ret: %d",
			  state ? "Step2" : "Step1",
...
int hdcp_3des_load_key(uint32_t *deshdcp_encrypted_key)
{
	...
	while (counter < DESHDCP_KEY_SIZE) {
		...
		if (RD_REG_32(hdcp.deshdcp_base_addr,
			      DESHDCP__DHDCP_DATA_H) != 0x0) {
			status = -HDCP_3DES_ERROR;
			HDCP_ERR("DESHDCP dummy read error\n");
		}
...
static int __init hdcp_init(void)
{
	...
	if (omapdss_get_version() == OMAPDSS_VER_OMAP4) {
		...
		if (!hdcp.hdmi_wp_base_addr) {
			HDCP_ERR("HDMI WP IOremap error\n");
			return -EFAULT;
		}
	} else {
