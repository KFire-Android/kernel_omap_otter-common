...
#define DEF_SELFTEST_ACCL_FULL_SCALE         (0 << 3) //set accel FSR to 0  (2 << 3)
#define DEF_SELFTEST_GYRO_SENS            (32768 / 250)
/* wait time before collecting data */
#define DEF_GYRO_WAIT_TIME          10  //Original value 10
#define DEF_ST_STABLE_TIME          200
#define DEF_PACKET_THRESH           200	//Original value 50
#define DEF_GYRO_THRESH             10
#define DEF_GYRO_SCALE              131
#define DEF_ST_PRECISION            1000
#define DEF_ST_ACCL_FULL_SCALE      2000UL   //set FSR to 0 +- 2Gs 8000UL
...
#define DEF_ACCEL_ST_SHIFT_DELTA     500	//Original value 140
#define DEF_GYRO_CT_SHIFT_DELTA      500	//Original value 140
...
void inv_recover_setting(struct inv_mpu_iio_s *st)
{
	...
	/* Recover Accel LPF */
	inv_i2c_single_write(st, REG_6500_ACCEL_CONFIG2, 0);
...
int inv_hw_self_test(struct inv_mpu_iio_s *st)
{
	...
	printk(KERN_INFO "Starting MPU6500 Sensitivity Self Test\n");
	...
	if (st->chip_config.is_asleep      ||
		...
		if (result) {
			printk(KERN_INFO "Failed to run Self Test: Setup conditions fail\n");
			return result;
		}
	}
	compass_result = 0;
	...
	if (result) {
		printk(KERN_INFO "Failed to run Self Test 2: Unable to get biases\n");
		goto test_fail;
	}
	...
	if (result) {
		printk(KERN_INFO "Failed to run Self Test 3: Unable to get ST_biases\n");
		goto test_fail;
	}
