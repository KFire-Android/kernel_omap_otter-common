...
{
	int ret;
	ret = omap_mux_init_signal("gpmc_ad11.gpio_35",
			OMAP_PULL_ENA | OMAP_PULL_UP | OMAP_MUX_MODE3 |
			OMAP_INPUT_EN | OMAP_PIN_OFF_INPUT_PULLUP);
	if (ret) {
		pr_err("%s: Can't mux signal.\n", __func__);
		return ret;
	}
	return i2c_register_board_info(4, omap4xx_i2c_bus4_touch_info,
