...
static int _pm_dbg_dump_clk(struct clk *clk, void *user)
{
	...
	if (clk->clkdm == parent_clkdm && clk->usecount)
...
static int _pm_dbg_dump_clkdm(struct clockdomain *clkdm, void *user)
{
	...
	if (clkdm->pwrdm.ptr == parent_pwrdm) {
		...
		if (usecount) {
			PM_DBG_PRINT(s, "    clockdomain:%s: %d\n",
				     clkdm->name, usecount);
			parent_clkdm = clkdm;
...
static int _pm_dbg_dump_pwrdm(struct powerdomain *pwrdm, void *user)
{
	...
	if (pwrdm->voltdm.ptr == parent_voltdm) {
		...
		if (usecount) {
			PM_DBG_PRINT(s, "  powerdomain:%s: %d\n", pwrdm->name,
				     usecount);
			parent_pwrdm = pwrdm;
...
{
	pr_info("powerdomain:%s: %d\n", pwrdm->name,
		atomic_read(&pwrdm->usecount));
	parent_pwrdm = pwrdm;
