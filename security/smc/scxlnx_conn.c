...
wait_for_unused:
	...
	SCXLNXCommReleaseSharedMemory(
			 &(pShmemMonitor->sAllocationContext),
			 pShmemDesc,
			 nFullCleanup);
 ...
 *Update the pBufferVAddr to L1 descriptors
...
static int SCXLNXConnMapShmem(
      ...
      u32 *pFlags,    /* flags for read-write access rights on the memory */
      int nInUserSpace,
      ...
      SCXLNX_SHMEM_DESC **ppShmemDesc)
{
	...
	dprintk(KERN_INFO
		...
		pShmemMonitor, *pBufferVAddr, *pBufferSize);
	...
	if (nError != 0) {
		dprintk(KERN_ERR
			"SCXLNXConnMapShmem(%p) : " \
			"SCXLNXCommFillDescriptorTable failed\n",
			SCXLNXConnMapShmem);
		goto error;
	}
					...
					pSharedMemoryMonitor)
{
	...
	SCXLNXInitializeCoarsePageTableAllocator(
	 &(pSharedMemoryMonitor->sAllocationContext));
	...
	pSharedMemoryMonitor->ppVmas =
		(struct vm_area_struct **) internal_get_zeroed_page(GFP_KERNEL);
	if (pSharedMemoryMonitor->ppVmas == NULL) {
...
/*FSY: must do this call?*/
	nError = SCXLNXConnValidateSharedMemoryBlockAndFlags(
			(void *) pMessage->sBody.sCreateDeviceContextMessage.
	...
	nError = SCXLNXConnMapShmem(
			...
			&(pMessage->sBody.
				...
				nSharedMemDescriptors[0]), &(nFlags),
			1,
			&(pMessage->sBody.
				sCreateDeviceContextMessage.
				nSharedMemStartOffset),
			&(pMessage->sBody.
				sCreateDeviceContextMessage.nSharedMemSize),
			&(pShmemDesc));
	...
	if (pAnswer->nSChannelStatus == S_SUCCESS) {
		...
		/* FSY, check that this call has to be changed or not */
		/*
		 * Return the buffer to map in the user space to the
		 * client if required
		 */
				...
				SCX_ANSWER_MESSAGE *pAnswer)
{
	...
	if (scxPublicCryptoIsShortcutedCommand(pConn,
		...
		case RPC_AES1_CODE:
			...
			break;
		case RPC_AES2_CODE:
			pHWALock = &g_SCXLNXDeviceMonitor.sAES2CriticalSection;
			break;
		case RPC_DES_CODE:
